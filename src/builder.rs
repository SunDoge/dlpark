...
mod tests {
    ...
    fn test_array_layout() {
        ...
        assert_eq!(boxed.shape, [1, 2, 3]);
        assert_eq!(boxed.strides, [6, 3, 1]);

        let raw_box = Box::into_raw(boxed);
        unsafe {
            let dlpack = Dlpack::new(&mut (*raw_box).managed_tensor as *mut DLManagedTensor).unwrap();
    ...
    fn test_slice_layout() {
        ...
        assert_eq!(boxed.shape, [2, 4, 8]);
        assert_eq!(boxed.strides, [32, 8, 1]);
