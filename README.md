...
- Use `python::from_dlpack(obj, stream, copy)` for one-step imports. CUDA and ROCm consumers that must create a stream after learning the device can instead construct `python::ImportRequest`, call `device()`, then finish with `import(stream, copy)`. The request caches protocol discovery and the device query, and verifies that the exported descriptor still reports that device. Both paths return `ImportedDlpack`, which owns either ABI, and negotiate in this order: DLPack C Exchange API, `__dlpack_device__` plus negotiated `__dlpack__`, then legacy no-argument `__dlpack__`.
