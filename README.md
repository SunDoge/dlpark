...
- Always use `python::from_dlpack(obj, stream, copy)` to import Python inputs. It accepts producer objects and existing capsules and returns `ImportedDlpack`, which owns either ABI. Its ordered negotiation tries the DLPack C Exchange API first, then `__dlpack_device__` plus negotiated `__dlpack__`, then the legacy no-argument `__dlpack__` protocol. `python::dlpack_device(obj)` uses the C Exchange API's borrowed tensor view when available before calling `obj.__dlpack_device__()`.
