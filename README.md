...
- PyO3 producers can subclass `python::DlpackProducer`. The base class owns the versioned tensor, implements `__dlpack__` and `__dlpack_device__`, enforces one-time consumption and zero-copy/device/version requests, and delegates only stream synchronization to a Rust callback. The CUDA and Metal Python examples use this path.
