...
fn main() -> Result<(), Whatever> {
    ...
    let dlpack = ManagedTensor::try_from(array).whatever_context("ndarray -> DLPack failed")?;
    ...
    let dlpack_back =
        ManagedTensor::try_from(tensor).whatever_context("candle::Tensor -> DLPack failed")?;
