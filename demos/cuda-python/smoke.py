...
exchange_producer = dlpark_cuda.CudaTensor.from_dlpack(producer)
torch_target = from_dlpack(producer)
torch_target_again = from_dlpack(producer)
torch_exchange_target = from_dlpack(exchange_producer)
...
assert exchange_producer.device_pointer == torch_exchange_target.data_ptr()
assert exchange_producer.device_pointer == producer.device_pointer
...
producer = dlpark_cuda.CudaTensor.from_dlpack(torch_source)
exchange_producer = dlpark_cuda.CudaTensor.from_dlpack(producer)
cupy_target = cp.from_dlpack(producer)
cupy_target_again = cp.from_dlpack(producer)
cupy_exchange_target = cp.from_dlpack(exchange_producer)
...
assert exchange_producer.device_pointer == cupy_exchange_target.data.ptr
assert exchange_producer.device_pointer == producer.device_pointer
